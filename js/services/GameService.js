marktai.service("GameService", ["$http", "$q", "$localStorage", "$websocket", "LoginService",  function($http, $q, $localStorage, $websocket, LoginService) { 
	...
	var arst = function(gameData, username) {
	...
	this.initws = function(gameid, open, change, close) {
		ws = $websocket.$new({'url': 'wss://www.marktai.com/T9/games/' + gameid + "/ws", 'protocols': [], 'subprotocols': ['base46']}); // instance of ngWebsocket, handled by $websocket service
